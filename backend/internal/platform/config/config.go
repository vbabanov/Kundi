...
func Load() (Config, error) {
	...
	cfg := Config{
		...
		AI: AIConfig{
			...
			AssistantLLMTimeout:               time.Duration(envInt("AI_ASSISTANT_LLM_TIMEOUT_SEC", 28)) * time.Second,
			AssistantPrimaryTimeout:           time.Duration(envInt("AI_ASSISTANT_PRIMARY_TIMEOUT_SEC", 25)) * time.Second,
